...
namespace IronPython.Runtime {
    ...
    public class FunctionCode : IExpressionSerializable {
        ...
        public PythonTuple co_freevars {
            get {
                return SymbolListToTuple(_lambda.FreeVariables != null ? _lambda.FreeVariables.Select(x => x.Name).ToList() : null);
            }
        }
        ...
        Expression IExpressionSerializable.CreateExpression() {
            return Expression.Call(
                ...
                Expression.NewArrayInit(
                    typeof(string),
                    Array.ConvertAll(_lambda.ParameterNames, (x) => Expression.Constant(x))
                ),
        ...
        private static Expression TupleToStringArray(PythonTuple tuple) {
            return tuple.Count > 0 ?
                (Expression)Expression.NewArrayInit(
                    typeof(string),
                    Array.ConvertAll(tuple._data, (x) => Expression.Constant(x))
                ) :
