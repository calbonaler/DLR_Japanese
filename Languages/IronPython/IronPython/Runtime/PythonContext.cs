...
namespace IronPython.Runtime {
    ...
    public sealed partial class PythonContext : LanguageContext {
        ...
        public PythonContext(ScriptDomainManager/*!*/ manager, IDictionary<string, object> options)
            ...
            if (_options.Frames) {
                var getFrame = BuiltinFunction.MakeFunction(
                    "_getframe", 
                    Array.ConvertAll(typeof(SysModule).GetMember("_getframeImpl"), (x) => (MethodBase)x), 
                    typeof(SysModule)
                );
            ...
            manager.AssemblyLoad += ManagerAssemblyLoaded;

            _mainThreadFunctionStack = Runtime.Operations.PythonOps.GetFunctionStack();
        }

        void ManagerAssemblyLoaded(object sender, AssemblyLoadEventArgs e) {
            _topNamespace.LoadAssembly(e.LoadedAssembly);
        }
		...
		public override ICollection<string> SearchPaths
		{
			...
			set
			{
				SetSystemStateValue("path", new List(value));
			}
		}
        ...
        internal string FormatStackTraceNoDetail(Exception e, ref bool printedHeader) {
            ...
            foreach (DynamicStackFrame frame in PythonExceptions.GetStackFrames(e, true)) {
                MethodBase method = frame.Method;
        ...
        private static string FrameToString(DynamicStackFrame frame) {
            return String.Format("  File \"{0}\", line {1}, in {2}",
                frame.FileName,
                frame.FileLineNumber == 0 ? "unknown" : frame.FileLineNumber.ToString(),
                frame.MethodName);
        }
