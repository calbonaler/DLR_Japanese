...
namespace IronPython.Runtime.Binding {
    ...
    class MetaMethod : MetaPythonObject, IPythonInvokable, IPythonConvertible {
        ...
        private DynamicMetaObject InvokeWorker(DynamicMetaObjectBinder/*!*/ callAction, DynamicMetaObject/*!*/[] args) {
            PerfTrack.NoteEvent(PerfTrack.Category.Binding, "Method Invoke " + args.Length);
            PerfTrack.NoteEvent(PerfTrack.Category.BindingTarget, "Method");
            ...
            if (Value.im_self == null) {
                    ...
                    call = new DynamicMetaObject(
                        Ast.Block(
                            ...
                            Ast.Dynamic(
                                ...
                                typeof(object),
                                ArrayUtils.Insert(PythonContext.GetCodeContext(callAction), ArrayUtils.Insert(func, args).Select(x => x != null ? x.Expression : null).ToArray())
                            )
                        ),
                ...
                call = new DynamicMetaObject(
                    Ast.Dynamic(
                        ...
                        typeof(object),
                        ArrayUtils.Insert(PythonContext.GetCodeContext(callAction), newArgs.Select(x => x != null ? x.Expression : null).ToArray())
                    ),
                    BindingRestrictions.Empty
                );
