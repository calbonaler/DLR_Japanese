...
namespace IronPython.Runtime.Types {
    ...
    sealed class NewTypeMaker {
        ...
        [MultiRuntimeAware]
        internal static readonly ConcurrentDictionary<NewTypeInfo, Type> _newTypes = new ConcurrentDictionary<NewTypeInfo, Type>();
        [MultiRuntimeAware]
        ...
        public static Type/*!*/ GetNewType(string/*!*/ typeName, PythonTuple/*!*/ bases) {
            ...
            Type ret = _newTypes.GetOrAdd(typeInfo,
                key => {
                    if (key.InterfaceTypes.Count == 0) {
                        // types that the have DynamicBaseType attribute can be used as NewType's directly, no need to create a new type unless we're adding interfaces
                        var attrs = key.BaseType.GetCustomAttributes(typeof(DynamicBaseTypeAttribute), false);
                        if (attrs.Length > 0)
                            return key.BaseType;
                    }
                    // creation code                    
                    return new NewTypeMaker(key).CreateNewType();
                });
        ...
        private Type CreateNewType() {
            string name = GetName();
            _tg = Snippets.DefinePublicType(TypePrefix + name, _baseType);
        ...
        private void EmitGetDict(ILGenerator gen) {
            gen.EmitFieldGet(_dictField);
        }

        private void EmitSetDict(ILGenerator gen) {
        ...
        private static void CallBaseConstructor(ConstructorInfo parentConstructor, ParameterInfo[] pis, ParameterInfo[] overrideParams, ILGenerator il) {
            il.EmitLoadArg(0);
#if DEBUG
            int lastIndex = -1;
#endif
        ...
        private void ImplementCTDOverride(MethodInfo m) {
            ...
            ILGenerator il = DefineExplicitInterfaceImplementation(m, out builder);
            il.EmitLoadArg(0);
        ...
        private void ImplementDynamicObject() {
            ...
            MethodInfo decl;
            MethodBuilder impl;
            ILGenerator il = DefineMethodOverride(MethodAttributes.Private, typeof(IDynamicMetaObjectProvider), "GetMetaObject", out decl, out impl);
            ...
            if (explicitDynamicObject) {
                _explicitMO = _tg.DefineField("__gettingMO", typeof(System.Threading.ThreadLocal<bool>), FieldAttributes.InitOnly | FieldAttributes.Private);
				...
				il.EmitPropertyGet(typeof(System.Threading.ThreadLocal<bool>), "Value");
                ...
                il.Emit(OpCodes.Ldfld, _explicitMO);
				il.EmitBoolean(true);
				il.EmitPropertySet(typeof(System.Threading.ThreadLocal<bool>), "Value");
        ...
        private void DefineHelperInterface(Type intf) {
            ...
            foreach (MethodInfo mi in mis) {
                MethodBuilder impl;
                ILGenerator il = DefineExplicitInterfaceImplementation(mi, out impl);
        ...
        /// </summary>
        private static void EmitBaseMethodDispatch(MethodInfo mi, ILGenerator il) {
            if (!mi.IsAbstract) {
                int offset = 0;
                if (!mi.IsStatic) {
                    il.EmitLoadArg(0);
        ...
        /// 
        /// </summary>
        private LocalBuilder EmitBaseClassCallCheckForProperties(ILGenerator il, MethodInfo baseMethod, string name) {
        ...
        /// </summary>
        private static void EmitConvertFromObject(ILGenerator il, Type toType) {
        ...
        private void CreateVTableEventOverride(MethodInfo mi, string name) {
            ...
            MethodBuilder impl;
            ILGenerator il = DefineMethodOverride(mi, out impl);
        ...
        /// 
        /// </summary>
        private static LocalBuilder EmitBaseClassCallCheckForEvents(ILGenerator il, MethodInfo baseMethod, string name) {
            Label instanceCall = il.DefineLabel();
            LocalBuilder callTarget = il.DeclareLocal(typeof(object));
        ...
        private void DefineVTableMethodOverride(MethodInfo mi, out MethodBuilder impl, out ILGenerator il) {
            if (mi.IsVirtual && !mi.IsFinal) {
        ...
        /// </summary>
        private LocalBuilder EmitNonInheritedMethodLookup(string name, ILGenerator il) {
            LocalBuilder callTarget = il.DeclareLocal(typeof(object));
        ...
        private MethodBuilder CreateSuperCallHelper(MethodInfo mi) {
            ...
            EmitBaseMethodDispatch(mi, method.GetILGenerator());
            return method;
        }
        ...
        private ILGenerator DefineExplicitInterfaceImplementation(MethodInfo baseMethod, out MethodBuilder builder) {
            ...
            Type[] baseSignature = baseMethod.GetParameters().Select(x => x.ParameterType).ToArray();
        ...
        private ILGenerator DefineMethodOverride(MethodAttributes extra, Type type, string name, out MethodInfo decl, out MethodBuilder impl) {
            decl = type.GetMethod(name);
            return DefineMethodOverride(extra, decl, out impl);
        }

        private ILGenerator DefineMethodOverride(MethodInfo decl, out MethodBuilder impl) {
            return DefineMethodOverride(MethodAttributes.PrivateScope, decl, out impl);
        }

        private ILGenerator DefineMethodOverride(MethodAttributes extra, MethodInfo decl, out MethodBuilder impl) {
            impl = ReflectionUtils.DefineMethodOverride(_tg, extra, decl);
            return impl.GetILGenerator();
        }
        ...
        /// </summary>
        private void EmitClrCallStub(ILGenerator il, MethodInfo mi, LocalBuilder callTarget) {
            int firstArg = 0;
        ...
        private static void EmitCodeContext(ILGenerator il, bool context) {
            if (context) {
                il.EmitLoadArg(1);
            } else {
    ...
    sealed class ReturnFixer {
        ...
        public void FixReturn(ILGenerator il) {
        ...
        public static ReturnFixer EmitArgument(ILGenerator il, ParameterInfo parameter, int index) {
