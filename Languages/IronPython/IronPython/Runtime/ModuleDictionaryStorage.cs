                        ...
                        case MemberTypes.Method:
                            if (!((MethodInfo)members[0]).IsSpecialName) {
                                value = BuiltinFunction.MakeFunction(
                                    name,
                                    Array.ConvertAll<MemberInfo, MethodInfo>(members, delegate(MemberInfo mi) { return (MethodInfo)mi; }),
                        ...
                        case MemberTypes.NestedType:
                            if (members.Length == 1) {
                                ...
                                for (int i = 1; i < members.Length; i++) {
                                    tt = TypeGroup.Merge(tt, (TypeTracker)MemberTracker.FromMemberInfo(members[i]));
                                }
