...
#if DEBUG
            PerfTrack.NoteEvent(PerfTrack.Category.Count, "Ruby: CLR member lookup failure cache " + (result ? "hit" : "miss"));
#endif
            return result;
        }
        ...
        private bool AddMethodsOverwriteExisting(ref Dictionary<Key<string, ValueArray<Type>>, ClrOverloadInfo> methods,
            ...
            for (int i = 0; i < newOverloads.Length; i++) {
                ...
                if (IsVisible(method.Attributes, method.DeclaringType, specialNameOnly)) {
                    var paramTypes = Key.Create(method.Name, new ValueArray<Type>(method.Parameters.Select(x => x.ParameterType).ToArray()));
                    if (methods == null) {
        ...
        public void BuildObjectConstructionNoFlow(MetaObjectBuilder/*!*/ metaBuilder, CallArguments/*!*/ args, string/*!*/ methodName) {
            ...
            if (isRubyInitializer || isLibraryInitializer && _isRubyClass) {
                ...
                if (typeof(Delegate).IsAssignableFrom(type)) {
                    ...
                    constructionOverloads = Array.ConvertAll(_factories, (d) => new ReflectionOverloadInfo(d.Method));
                } else {
