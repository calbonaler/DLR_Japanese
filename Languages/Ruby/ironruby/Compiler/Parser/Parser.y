...
expression_statement: 
    ...
    | lhs ASSIGNMENT compound_rhs
        {
            $$ = new ParallelAssignmentExpression(new CompoundLeftValue(new List<LeftValue>() { $1 }, null, @1), $3, @$);
        }
...
compound_lhs: 
    ...
    | LEFT_EXPR_PARENTHESIS compound_lhs RIGHT_PARENTHESIS
        {
            $$ = new CompoundLeftValue(new List<LeftValue>() { $2 }, null, @$);
        }
...
undef_list:
      method_name_or_symbol
        {
            $$ = new List<Identifier>() { new Identifier($1, @1) };
        }
   | undef_list COMMA 
        {
            _tokenizer.LexicalState = LexicalState.EXPR_FNAME;
...
block_parameters:
      lhs 
        { 
            $$ = new CompoundLeftValue(new List<LeftValue>() { $1 }, null, @1); 
        }
    | compound_lhs 
        { 
            $$ = $1; 
...
default_parameter_list: 
      default_parameter
        {
            $$ = new List<SimpleAssignmentExpression>() { $1 };
        }
