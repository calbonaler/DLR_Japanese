        ...
        private MSA.Expression/*!*/ TransformRead(AstGenerator/*!*/ gen, int opKind) {
            ...
            if (constantQualifier != null) {
                ...
                if (constant.Qualifier != null) {
                    // {expr}::A::B
                    return constant.MakeExpressionQualifiedRead(gen, opKind, Enumerable.Reverse(names).ToArray());
                } else {
                    // A::B
                    return MakeCachedRead(gen, opKind, constant.IsGlobal, true, Ast.Constant(Enumerable.Reverse(names).ToArray()));
                }
