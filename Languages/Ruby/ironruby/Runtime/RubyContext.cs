...
namespace IronRuby.Runtime {
    ...
    public sealed class RubyContext : LanguageContext {
        ...
        public RubyContext(ScriptDomainManager/*!*/ manager, IDictionary<string, object> options)
            ...
            manager.AssemblyLoad += AssemblyLoaded;
            foreach (Assembly asm in manager.GetLoadedAssemblies()) {
        ...
        private void AssemblyLoaded(object sender, AssemblyLoadEventArgs e) {
            _namespaces.LoadAssembly(e.LoadedAssembly);
        }
        ...
        internal RubyClass/*!*/ GetOrCreateInstanceSingleton(object obj, Action<RubyModule> instanceTrait, Action<RubyModule> classTrait, 
            ...
            using (ClassHierarchyLocker()) {
                ...
                if (!(obj is IRubyObject)) {
                    PerfTrack.NoteEvent(PerfTrack.Category.Count, "Non-IRO singleton created " + immediate.NominalClass.Name);
                }
            }
		...
		public override ErrorSink CompilerErrorSink { get { return _runtimeErrorSink; } }
        ...
        public override ScriptCode/*!*/ LoadCompiledCode(Delegate/*!*/ method, string path, string customData) {
            ...
            SourceUnit su = new SourceUnit(this, TextContentProvider.Null, path, SourceCodeKind.File);
        ...
        private void ExecuteShutdownHandlers() {
            ...
            while (true) {
                ...
                lock (ShutdownHandlersLock) {
					...
					handlers = Enumerable.Reverse(_shutdownHandlers).ToArray();
...
",
     ...
     "N/A", "N/A"
		...
		public override ICollection<string> SearchPaths
		{
			get
			{
