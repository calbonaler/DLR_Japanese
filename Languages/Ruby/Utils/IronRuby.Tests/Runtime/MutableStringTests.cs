...
namespace IronRuby.Tests {
    ...
    public partial class Tests {
        ...
        public void MutableString_Append() {
            ...
            y = MS("α", RubyEncoding.UTF8);
            Assert(x.Append(y).ToByteArray().SequenceEqual(Utils.Concatenate(invalid_utf8, valid_utf8)));

            x = MS(invalid_utf8, RubyEncoding.UTF8);
            y = MS("α", RubyEncoding.UTF8);
            Assert(y.Append(x).ToByteArray().SequenceEqual(Utils.Concatenate(valid_utf8, invalid_utf8)));

            x = MS(invalid_utf8, RubyEncoding.UTF8);
            Assert(x.Append(x).ToByteArray().SequenceEqual(Utils.Concatenate(invalid_utf8, invalid_utf8)));


            x = MS(invalid_utf8, RubyEncoding.UTF8);
            y = MS("βαγ", RubyEncoding.UTF8);
            Assert(x.Append(y, 1, 1).ToByteArray().SequenceEqual(Utils.Concatenate(invalid_utf8, valid_utf8)));

            x = MS(invalid_utf8, RubyEncoding.UTF8);
            y = MS("α", RubyEncoding.UTF8);
            Assert(y.Append(x, 1, 2).ToByteArray().SequenceEqual(Utils.Concatenate(valid_utf8, new byte[] { 0x85, 0x9c })));

            x = MS(invalid_utf8, RubyEncoding.UTF8);
            Assert(x.Append(x, 1, 2).ToByteArray().SequenceEqual(Utils.Concatenate(invalid_utf8, new byte[] { 0x85, 0x9c })));
        }
        ...
        private void Test_Translate(
            ...
            Assert(b.SequenceEqual(expected));
        }
        ...
        public void MutableString_Bytes1() {
            ...
            Assert(bs.ToArray().SequenceEqual(Encoding.UTF8.GetBytes("αβ")));
            ...
            Assert(bs.ToArray().SequenceEqual(new byte[] { (byte)'a', (byte)'b' }));
        ...
        public void MutableString_ChangeEncoding1() {
            ...
            Assert(s.ToByteArray().SequenceEqual(alpha) && s.Encoding == RubyEncoding.Binary && s.IsBinaryEncoded && s.GetHashCode() == hc);
            s.ChangeEncoding(RubyEncoding.KCodeSJIS, true);
            Assert(s.ToString() == "ﾎｱ" && s.Encoding == RubyEncoding.KCodeSJIS && !s.IsBinaryEncoded && s.GetHashCode() == hc);
            ...
            s.ChangeEncoding(SJIS, true);
            Assert(s.ToByteArray().SequenceEqual(SJIS.StrictEncoding.GetBytes("α")));
