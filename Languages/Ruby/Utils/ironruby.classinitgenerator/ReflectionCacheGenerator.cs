...
internal sealed class ReflectionCacheGenerator : Generator {
    ...
    public void Generate() {
		...
		var methods = reflectedMethods.OrderBy(x => x.Key).ToArray();
		var fields = reflectedFields.OrderBy(x => x.Key).ToArray();
        if (_anyError) {
            ...
            switch (existing.MemberType) {
                case MemberTypes.Method:
                    Console.WriteLine("ERROR: Emitted methods should not have overloads: \n\t{0}\n\t{1}",
                         ReflectionUtils.FormatSignature((MethodInfo)(object)existing),
                         ReflectionUtils.FormatSignature((MethodInfo)(object)member));
                    break;
    ...
    private void GenerateMethods(IList<KeyValuePair<string, MethodInfo>>/*!*/ methods) {
        foreach (var entry in methods) {
            ...
            if (!method.IsGenericMethod && method.IsStatic && Array.TrueForAll(ps, (p) => !p.ParameterType.IsByRef) &&
                !((EmittedAttribute)method.GetCustomAttributes(typeof(EmittedAttribute), false)[0]).UseReflection) {

                Type[] types = ps.Select(x => x.ParameterType).ToArray();
                string delegateType;
                ...
                _output.WriteLine("public static MethodInfo/*!*/ {0} {{ get {{ return _{0} ?? (_{0} = CallInstruction.Cache{3}{4}({1}.{2})); }} }}",
                    ...
                    method.Name,
                    delegateType,
                    ReflectionUtils.FormatTypeArgs(types, Generator.TypeNameDispenser)
                 );
