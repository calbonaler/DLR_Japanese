...
namespace Microsoft.Scripting.Actions {
    ...
    public partial class DefaultBinder : ActionBinder {
        ...
        DynamicMetaObject MakeGetMemberTarget(GetMemberInfo getMemInfo, DynamicMetaObject target) {
            ...
            if (members.Count == 0) {
                if (typeof(TypeTracker).IsAssignableFrom(type)) {
                    ...
                    var x = ((TypeTracker)target.Value).Type;
        ...
        void MakeGenericBody(GetMemberInfo getMemInfo, Type type, MemberGroup members, DynamicMetaObject instance) {
            var bestMember = members[0];
            if (members.Count > 1) {
                // if we were given multiple members pick the member closest to the type...                
                var bestMemberDeclaringType = members[0].DeclaringType;
                for (int i = 1; i < members.Count; i++) {
                    var mt = members[i];
                    if (!IsTrackerApplicableForType(type, mt))
                        continue;
                    if (members[i].DeclaringType.IsSubclassOf(bestMemberDeclaringType) || !IsTrackerApplicableForType(type, bestMember)) {
        ...
        void MakeTypeBody(GetMemberInfo getMemInfo, Type type, MemberGroup members) {
            var typeTracker = (TypeTracker)members[0];
            for (int i = 1; i < members.Count; i++)
